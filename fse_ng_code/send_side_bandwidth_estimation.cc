...
SendSideBandwidthEstimation::SendSideBandwidthEstimation(
  ...
  fseNgFlow_ = FseNg::Instance().RegisterRateFlow(min_bitrate_configured_,
                                                   1, *this);
  // Added by TOBIAS
}
...
void SendSideBandwidthEstimation::UpdateTargetBitrate(DataRate new_bitrate,
  ...
  if (fse_is_enabled_) {
    ...
    FseNg::Instance().FseNgSrtpUpdate(fseNgFlow_, current_target_, new_bitrate, GetUpperLimit(),
...
void SendSideBandwidthEstimation::FseUpdateTargetBitrate(DataRate new_bitrate,
                                                         Timestamp at_time) {
  current_target_ = new_bitrate;
  RTC_LOG(LS_INFO) << "TOBIAS(fse_r_update) " << fseFlow_->Id() << "-FSE_R "
