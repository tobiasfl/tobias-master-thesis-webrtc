...
void FseNgWindowBasedFlow::UpdateCc(uint32_t max_cwnd) {
  //TODO: check here if the flow has started first
...
FseNgRateFlow::FseNgRateFlow(
    ...
    : FseFlow(id, priority),
      ...
      prev_fse_rate_(initial_bit_rate),
      flow_cc_(flow_cc) {
...
void FseNgRateFlow::UpdateCc(Timestamp at_time) {
  flow_cc_.FseUpdateTargetBitrate(fse_rate_, at_time);
}
...
void FseNgRateFlow::SetFseRate(DataRate new_rate) {
    fse_rate_ = new_rate;
}


DataRate FseNgRateFlow::PrevFseRate() const{
    return prev_fse_rate_;
}

void FseNgRateFlow::SetPrevFseRate(DataRate new_rate) {
    prev_fse_rate_ = new_rate;
}
